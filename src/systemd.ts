...
export class SystemDDeployer extends BaseDeployer {
    ...
    async createSystemDFile(servicePath: string, serviceName: string, artifactRepoDir: string) {
        ...
        contents = contents.replace(EXE_KEY, this.replaceVars(`bash -ilc "${this.manifest.deploy.cmdPrefix} ${this.manifest.deploy.preRunFlags} ${this.manifest.deploy.path} ${this.manifest.deploy.postRunFlags}"`, artifactRepoDir));
